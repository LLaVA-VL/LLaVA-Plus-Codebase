...
def main():
    ...
    # random points
    points = np.random.rand(1, 3, 2).tolist()
    ...
    for mask_rle in masks_rle:
        ...
        annotated_frame = show_mask(mask, image_source)
    ...
    for i in range(points.shape[1]):
        ...
        annotated_frame = cv2.circle(annotated_frame, (int(points[0, i, 0] * w), int(points[0, i, 1] * h)), 5, color, -1)
